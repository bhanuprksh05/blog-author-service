...
import { invalidateChacheJob } from "../utils/producer";
...
export const createBlog = TryCatch(async (req: UserRequest, res) => {
  ...
  if(file){
    ...
    result = 
        await sql`INSERT INTO blogs (title, description, blogcontent,category, author) VALUES (${title}, ${description},${blogcontent},${category},${req.user?._id}) RETURNING *`;
...
export const updateBlog = TryCatch(async (req: UserRequest, res) => {
  const { id } = req.params;
  const { title, description, blogcontent, category } = req.body;
...
export const deleteBlog = TryCatch(async (req: UserRequest, res) => {
